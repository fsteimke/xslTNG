...
<xsl:stylesheet xmlns:db="http://docbook.org/ns/docbook"
  xmlns:err="http://www.w3.org/2005/xqt-errors" xmlns:f="http://docbook.org/ns/docbook/functions"
...
|=================================================================================== -->
  ...
  <xsl:key name="glossary-entry" match="db:glossary//db:glossentry" use="fp:baseform(db:glossterm)"/>
  ...
  <xsl:key name="bibliography-entry" match="db:biblioentry[db:abbrev] | db:bibliomixed[db:abbrev]"
    use="db:abbrev ! normalize-space(.)"/>

  <!-- ============================================================ -->

  <xsl:variable name="v:pi-db-attributes-are-uris" as="xs:string*" select="
      ('glossary-collection', 'bibliography-collection',
...
|===================================================================================  -->
  ...
  <xsl:variable name="vp:pi-match" select="'^.*?(\c+)=[''&quot;](.*?)[''&quot;](.*)$'"/>
  ...
  <xsl:function name="f:pi" as="xs:string*" visibility="public">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:sequence select="
            fp:pi-from-list($context/processing-instruction('db'),
            $property, $default)"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:function>
  ...
  <xsl:function name="fp:pi-from-list" as="xs:string*">
    ...
    <xsl:variable name="value" select="f:pi-attributes($pis)/@*[local-name(.) = $property]/string()"/>
  ...
  <xsl:function name="f:pi-attributes" as="element()?">
    ...
    <xsl:variable name="attributes" select="fp:pi-attributes($pis, map {})"/>
  ...
  <xsl:function name="fp:pi-pi-attributes" as="map(*)">
    ...
    <xsl:choose>
      <xsl:when test="matches($text, $vp:pi-match)">
        ...
        <xsl:variable name="avalue-resolved" select="
            if ($aname = $v:pi-db-attributes-are-uris)
            then
              tokenize(normalize-space($avalue), '\s+') ! resolve-uri(., base-uri($pi))
            else
              $avalue"/>

        <xsl:variable name="list-value" select="
            if (map:contains($pimap, $aname))
            then
              (map:get($pimap, $aname), $avalue-resolved)
            else
              $avalue-resolved"/>
        <xsl:sequence select="fp:pi-pi-attributes(map:put($pimap, $aname, $list-value), $pi, $rest)"
        />
      </xsl:when>
      <xsl:otherwise>
        <xsl:sequence select="$pimap"/>
...
|=================================================================================== -->
  ...
  <xsl:function name="f:glossentries" as="element(db:glossentry)*">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:message select="
            'Warning: f:glossentries must not be called with '
...
|=================================================================================== -->
  ...
  <xsl:template match="
      db:bibliomixed[empty(node() except db:abbrev)]
      | db:biblioentry[empty(node() except db:abbrev)]" mode="mp:strip-empty-biblioentries">
    <!-- discard -->
  </xsl:template>
  ...
  <xsl:function name="f:biblioentries" as="element()*">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:message select="
            'Warning: f:biblioentries must not be called with '
            || local-name($term) || ' as $term.'"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:function>
  ...
  <xsl:function name="f:flatten-path" as="xs:anyURI">
    ...
    <xsl:variable name="path" select="
        ...
        then
          ...
          substring-after($path, $windows-drive)"/>
...
<!-- ===================================================================================
|    Issue In Oxygen version 26.1 or less: fp:contains-token#2 is unknown
|=================================================================================== -->

<xsl:function name="fp:contains-token" as="xs:boolean" visibility="public">
  <xsl:param name="input" as="xs:string*"/>
  <xsl:param name="token" as="xs:string"/>
  <xsl:variable name="t" as="xs:string" select="normalize-space($token)"/>
  <xsl:choose>
    <xsl:when test="function-available('contains-token',2)">
...
<!--  ==================================================================================
|     user friendly path function
|==================================================================================  -->  
<xsl:function name="fp:path" as="xs:string?">
  <xsl:param name="node" as="node()?"/>
  <xsl:variable name="p" as="xs:string?" select="path($node)"/>
  <xsl:choose>
    <xsl:when test="$node instance of element() and $p gt ''">
      <xsl:variable name="prefix" as="xs:string?"
        select="in-scope-prefixes($node)[namespace-uri-for-prefix(., $node) eq 'http://docbook.org/ns/docbook']"/>
      <xsl:choose>
        <xsl:when test="$prefix gt ''">
          <xsl:sequence select="replace($p, 'http://docbook.org/ns/docbook', $prefix)"/>
        </xsl:when>
        <xsl:otherwise>
